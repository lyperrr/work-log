...
export function DashboardPage({ onNavigate }) {
  ...
  const [kunjunganList] = useState(() => apiService.getKunjunganList());
  const [paketList] = useState(() => apiService.getPaketList());
  ...
  const lunasVisits = kunjunganList.filter((k) => k.status === 'lunas');

  const todayIncome = lunasVisits
    .filter((k) => k.tanggal_kunjungan === todayStr)
    .reduce((sum, k) => sum + (Number(k.biaya) || 0), 0);

  const weekIncome = lunasVisits
    .filter((k) => k.tanggal_kunjungan >= startOfWeek)
    .reduce((sum, k) => sum + (Number(k.biaya) || 0), 0);

  const monthIncome = lunasVisits
    .filter((k) => k.tanggal_kunjungan >= startOfMonth)
    .reduce((sum, k) => sum + (Number(k.biaya) || 0), 0);

  const pendingCount = kunjunganList.filter((k) => k.status === 'menunggu').length;
  const unpaidCount = kunjunganList.filter((k) => k.status === 'belum bayar').length;
  const activePaketCount = paketList.filter((p) => p.status_paket === 'aktif').length;
  ...
  const chartData = Array.from({ length: 7 }).map((_, i) => {
    ...
    const dayTotal = lunasVisits
      .filter((k) => k.tanggal_kunjungan === dateStr)
      .reduce((sum, k) => sum + (Number(k.biaya) || 0), 0);
    ...
    <div className="space-y-6 animate-in fade-in-50">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 sm:p-5 md:p-6 rounded-3xl bg-linear-to-r from-primary/15 via-primary/10 to-cyan-500/10 border border-primary/20 shadow-xs overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <Badge variant="outline" className="text-xs font-bold text-primary max-w-[200px] sm:max-w-xs truncate">
            <Sparkles className="size-3.5 mr-1.5 text-amber-500 shrink-0 animate-pulse" />
            <span className="truncate">{currentUser?.email || 'Akun Freelance'}</span>
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-amber-500/10 border border-amber-500/25 p-4 rounded-2xl text-center shadow-xs hover:scale-102 transition-transform">
        ...
        <div className="bg-rose-500/10 border border-rose-500/25 p-4 rounded-2xl text-center shadow-xs hover:scale-102 transition-transform">
        ...
        <div className="bg-primary/10 border border-primary/25 p-4 rounded-2xl text-center shadow-xs hover:scale-102 transition-transform">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="size-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-black text-base shrink-0">
                  {k.nama_pasien.charAt(0).toUpperCase()}
                </div>
                <div>
