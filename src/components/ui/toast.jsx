...
function ToastViewport({
  ...
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed inset-x-3 top-[72px] z-[9999] mx-auto w-[calc(100vw-1.5rem)] max-w-md outline-none sm:top-20 sm:right-5 sm:left-auto sm:mx-0 sm:max-w-sm",
        className
      )}
      {...props} />
  );
...
function ToastContent({
  ...
  return (
    <ToastPrimitive.Content
      ...
      className={cn(
        "flex min-h-13 w-full items-center gap-3 p-3.5 sm:p-4 text-sm sm:text-base font-bold transition-opacity duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] data-behind:opacity-0 data-expanded:opacity-100",
        className
      )}
      {...props} />
  );
